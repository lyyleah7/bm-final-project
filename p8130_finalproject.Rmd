...
lawsuit =
  ...
  mutate(
    dept = factor(dept, levels = c(1,2,3,4,5,6), labels = c("biochemistry", "physiology","genetics","pediatrics","medicine","surgery"), ordered = TRUE),
    gender = factor(gender, levels = c(0,1), labels = c("female","male"), ordered = TRUE),
    clin = factor(clin, levels = c(0,1), labels = c("research emphasis","clinical emphasis"), ordered = TRUE),
    cert = factor(cert, levels = c(0,1), labels = c("not certified", "board certified"),ordered = TRUE),
    rank = factor(rank, levels = c(1,2,3), labels = c("assistant", "associate","full professor"), ordered = TRUE)
  )
