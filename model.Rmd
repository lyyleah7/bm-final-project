...
assistant_fit = lm(lg_aveg_sal ~ gender + dept + clin +cert +exper, data = assistant_df)
...
associate_fit = lm(lg_aveg_sal ~ gender + dept + clin +cert +exper, data = associate_df)
...
full_df =
  lawsuit_clean %>% 
  filter(rank == "full")
full_fit = lm(lg_aveg_sal ~ gender + dept + clin +cert +exper, data = full_df)
summary(full_fit)
...
experlarge_df = 
  lawsuit_clean %>% 
  filter(exper >= 10)
experlarge_fit = lm(lg_aveg_sal ~ gender + dept + clin +cert + rank, data = experlarge_df)
...
expersmall_df = 
  lawsuit_clean %>% 
  filter(exper < 10)
expersmall_fit = lm(lg_aveg_sal ~ gender + dept + clin +cert + rank, data = expersmall_df)
