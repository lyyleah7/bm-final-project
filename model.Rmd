...
confounder_fit = lm(lg_aveg_sal ~ gender + dept + clin + cert + prate + exper, data = lawsuit_clean)
summary(confounder_fit)

vif(confounder_fit)
#the vif of prate is larger than 10 ,implying serious collinearity, then we drop prate

conf_final_fit = lm(lg_aveg_sal ~ gender + dept + clin + cert + exper , data = lawsuit_clean)
...
```{r}
inter1 = lm(lg_aveg_sal ~ gender * dept + clin + cert + exper , data = lawsuit_clean)
summary(inter1)
#there is no interaction b/w gender and dept

inter2 = lm(lg_aveg_sal ~ gender*clin + dept  + cert + exper , data = lawsuit_clean)
summary(inter2)

#there is interaction b/w gender and clin

inter3 = lm(lg_aveg_sal ~ gender*cert + clin + dept + exper , data = lawsuit_clean)
summary(inter3)

# there is no interaction b/w gender and cert

inter4 = lm(lg_aveg_sal ~ gender*exper + cert + clin + dept , data = lawsuit_clean)
summary(inter4)

# there is interaction b/w gender and exper

inter5 = lm(lg_aveg_sal ~ gender*prate + dept + clin + cert + exper , data = lawsuit_clean)
summary(inter5)
...
# stepwise
```{r}
final1_fit = lm(lg_aveg_sal ~ dept + gender*clin + cert + gender*exper , data = lawsuit_clean)
summary(final1_fit)
...
final2_fit = lm(lg_aveg_sal ~ dept + gender*clin + cert + gender*exper, data = lawsuit_nooutlier)
summary(final2_fit)
par(mfrow = c(2, 2))
plot(final2_fit)
```
